...
| M5 ⚙ in progress | Perf campaign to G2: ceiling probes, redundancy elimination first (codec-optimize order), GFNI/AVX-512 evaluated behind detection | **GFNI phase done** (07020e4 + LEDGER M5): stable intrinsics confirmed on 1.95; affine matrices proven vs ISA-L's own table; **safe-core Rust beats Intel's dispatched GFNI asm — S2 1.09× (3/3 pairwise), 1 MiB 1.26× (non-overlapping), interleaved cross-implementation** ; census 100% on `x86_64/avx2_gfni`. One brick reverted (matrix-preload, measured-worse interleaved — recorded). **G2 remaining**: the 4 KiB cell (0.57× — small-stripe per-call overhead) breaches the 1.15× bound |
