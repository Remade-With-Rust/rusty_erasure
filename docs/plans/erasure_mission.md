...
| Gate | Bar | Status |
|---|---|---|
| G1 conformance | byte-identical vs ISA-L on every legal cell of the config matrix (k ≤ 32, p ≤ 8, Vandermonde within its limits + Cauchy everywhere), all golden vectors, recovery for every loss pattern ≤ p on sampled configs, update-sequence ≡ one-shot | not started |
| G2 speed vs ISA-L | on the rig, matched ISA arm (AVX2 vs AVX2): ≥ parity on S2; no scenario worse than 1.15×; beat on ≥ half the cells. Honest reporting per cell — parity is a fine v1 story (rusty_alloc precedent), a false "beats" claim is not | not started |
...
| M3 | Validation + no-panic hardening; the typed public API (§5) + ISA-L compat layer; ISA-L's own test suite ported through the compat layer | ported ISA-L tests green; fuzzers on all public entries clean; Miri clean; scalar perf baseline in the ledger (with null-arm floor) |
| M4 | x86-64 SIMD: SSSE3 + AVX2 nibble kernels, 1..6-row fused, runtime dispatch, scalar tails; the reach census instrument; **first ERASCORP perf run vs ISA-L** | every kernel `*_matches_scalar` over random + edge inputs; census 100% on x86-64; G1 cells green; S2 within 1.5× of ISA-L AVX2 (calibration point, not the ship bar) |
| M5 | Perf campaign to G2: ceiling probes, redundancy elimination first (codec-optimize order), GFNI/AVX-512 evaluated behind detection | G2 bars met on the rig; every kept brick has before/after + method line in the ledger; every reverted brick recorded with which-kind-of-revert |
| M6 | aarch64 NEON mirror + wasm32 SIMD128 kernel; `--no-default-features` and cross-target CI | NEON matches scalar on aarch64 CI; wasm kernel matches scalar under wasmtime/node; per-arch census; browser demo encodes/recovers a stripe |
| M7 | RAID module (`xor`/`pq` + checks), rayon multi-stripe feature, streaming API; **the consumer swap**: `spacedb-durability` drops `reed-solomon-erasure = "6"` for `rusty_erasure` (its seam is two functions — `encode_snapshot`/`reconstruct_snapshot`) | RAID byte-identical vs ISA-L; S9/S10 in the ledger; Deputy `snapshot` → delete shards → `restore` round-trips on a real vault through rusty_erasure; existing spacedb snapshots still reconstruct (compat gate) |
