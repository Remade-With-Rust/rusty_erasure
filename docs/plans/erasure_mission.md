...
| G2 speed vs ISA-L | on the rig, matched ISA arm (AVX2 vs AVX2): ≥ parity on S2; no scenario worse than 1.15×; beat on ≥ half the cells. Honest reporting per cell — parity is a fine v1 story (rusty_alloc precedent), a false "beats" claim is not | ✅ **MET** (LEDGER ten-brick campaign, all interleaved): S1 **1.09×** · S2 **1.33×** · S3 **1.64×** · S5a **1.06×** · S5b 0.99× (tie) · 4K-k10 **1.05×** — beat on 5 of 6, worst cell 0.99×, no cell near the 1.15× bound. Non-encode gaps tracked separately: S9 pq 0.744×, planned recovery ~0.76× (backlog with named mechanisms) |
