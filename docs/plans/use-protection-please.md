...
outputs below); survey for the release/ops gates that still need a tagged release to
...
| H-12 | ★ SBOM generated and published with releases | Incomplete | The crates are published (0.4.0 on crates.io) but no git tag has been cut, so there is no GitHub Release to attach artifacts to. `release.yml` is written AND PROVEN — its first run went green across all four platforms, producing auditable binaries, the CycloneDX SBOM, verified hardening and provenance attestation. Blocked only on a tag, which needs the owner's signing key | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-23 | ★ Tests pass under Miri | Completed | Two runs in the CI `miri` job. (1) Our own crates: `cargo +nightly miri test -p rusty_erasure-core -p rusty_erasure`, zero UB, zero leaks. (2) **The unsafe crate with the SIMD actually reached**, under `-C target-feature=+ssse3,+avx2` — the run prints `testing x86_64/avx2` and reports no UB (504 s locally). **This gate was VACUOUS until 2026-08-28**: under Miri `is_x86_feature_detected!` returns false, so dispatch fell back to scalar and the job went green having executed none of the unsafe code it exists to check — the "fast path nothing calls" failure mode applied to a safety gate, and the important half, since Miri is the stated cover for a memory fault that changes no output byte. GFNI stays off deliberately: Miri has no model for `GF2P8AFFINEQB` and would abort rather than check. The three `klauspost_compat` tests are `cfg_attr(miri, ignore)` because `reed-solomon-erasure` 6.0.0 has its own Stacked Borrows violation (`galois_8.rs:166`) in a dev-only dependency — our frames are absent from that stack | |
