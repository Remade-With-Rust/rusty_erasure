...
pub enum Level {
    /// 16 bytes per step, `pshufb`.
    Ssse3,
    /// 32 bytes per step, `vpshufb` on both lanes.
    Avx2,
}
...
pub fn kernels() -> Option<Kernels> {
    #[cfg(target_arch = "x86_64")]
    {
        ...
        *CHOICE.get_or_init(|| {
            kernels_at(Level::Avx2).or_else(|| kernels_at(Level::Ssse3))
        })
    }
...
/// Used by the oracle tests to exercise every level the box supports;
/// production wants [`kernels`].
...
pub(crate) mod imp {
    ...
    pub(crate) fn mad_avx2(tbl: &[u8; TABLE_BYTES], src: &[u8], dest: &mut [u8]) {
    ...
    pub(crate) fn mad_ssse3(tbl: &[u8; TABLE_BYTES], src: &[u8], dest: &mut [u8]) {
        assert_eq!(src.len(), dest.len(), "mad length mismatch");
        ACCEL_CENSUS_BYTES.fetch_add(src.len() as u64, Ordering::Relaxed);
